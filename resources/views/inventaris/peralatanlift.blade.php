...
<html lang="en">
    ...
    <ol class="breadcrumb bg-light-lighten p-2">
        ...
        <li class="breadcrumb-item active" aria-current="page">Peralatan Lift</li>
    </ol>
</nav>
                                </div>
                            </div>
                                ...
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <div class="row mb-2">
                                            <div class="row mb-2">
                                            <div class="col-sm-4">
                        @if(count($inventaris_peralatanlift) > 1)
                            <a target="_blank" href="../qrall7/">
                        ...
                        @endif
                                            ...
                                            <div class="col-sm-8">
                                                <div class="text-sm-end">
                                                    ...
                                                    <a href="../exportinventarisperalatanlift"><button type="button" data-bs-toggle="modal" data-bs-target="#"  class="btn btn-danger mb-2 me-1"><i class="mdi mdi-file-excel me-2"></i>Excel</button></a>
                                                    <?php
                                                        if (session('data')->role == 'Corporation') {

                                                        }else {
                                                            ?>
                                                            <button type="button" data-bs-toggle="modal" data-bs-target="#addperalatanlift"  class="btn btn-success mb-2 me-1"><i class="mdi mdi-plus-circle me-2"></i>Tambah</button>
                                                            <?php
                                                        }
                                                        ?>
                                            ...
                                            <div class="tab-pane show active" id="buttons-table-preview">
                                                ...
                                                <table id="datatable-buttons" class="table table-striped dt-responsive nowrap w-100">
                                                    ...
                                                    <tbody>
                                                        <?php
                                                        $nourut = 0;
                                                        foreach($inventaris_peralatanlift as $user) {
                                                            $nourut++;
                                                            ...
                                                            if ($ket == null || $ket == '' ) {
                                                             ...
                                                             $merek = $user->merek_peralatanlift;
                                                             $jenis = $user->jenis_peralatanlift;
                                                             $tipe = $user->tipe_peralatanlift;

                                                            ?>
                                                            <tr>
                                                            <td><?= $nourut ?></td>
                                                            <td><?= $no ?></td>
                                                            <td><?= $user->nama_peralatanlift ?></td>
                                                            <td><?= $jenis ?></td>
                                                            <td><?= $user->merek_peralatanlift ?></td>
                                                            <td><?= $user->kode_peralatanlift?></td>
                                                            ...
                                                            <a data-bs-toggle="modal" data-bs-target="#previewperalatanlift-<?=$user->id_peralatanlift?>" data-id='<?= $user->id_peralatanlift?>' class="action-icon"> <i class="mdi mdi-eye"></i></a>
                                                            <?php
                                                        if (session('data')->role == 'Corporation') {
                                                            ...
                                                            <a onclick="loaddata(<?=$nourut?>);" data-bs-toggle="modal" data-bs-target="#editperalatanlift" class="action-icon" id="editbutton-<?=$nourut?>" data-petugasdua='<?=$user->id_petugas2?>'  data-id='<?= $user->id_peralatanlift ?>' data-nama='<?= $user->nama_peralatanlift?>' data-jenis='<?=$jenis?>'
data-merek='<?=$user->merek_peralatanlift?>'
data-tipe='<?=$user->tipe_peralatanlift?>'
data-spesifikasi='<?=$user->spesifikasi_peralatanlift?>'
...
data-keterangan='<?= isset($user->keterangan) ? $user->keterangan : '-'; ?>'  
                                                            ...
                                                            <a onclick="return swallnya(<?=$user->id_peralatanlift?>)" href="#" class="action-icon"> <i class="mdi mdi-delete"></i></a>
                                                            <?php
                                                        }
                                                        ?>

                                                            <a target="_blank" href="../qr2/<?=$tanggalbarang->format("Y")?>/<?=$tanggalbarang->format("m")?>/<?= $user->kode_peralatanlift?>/<?= $user->id_user ?>/<?= $user->usernamanya ?>/<?=$no?>/<?=str_replace('/', ' atau ', $user->nama_peralatanlift)?>/<?=str_replace('/', ' atau ', $jenis)?>/<?=str_replace('/', ' atau ', $merek)?>/<?=str_replace('/', ' atau ', $tipe)?>/<?=str_replace('/', ' atau ', $ket)?>" class="action-icon"> <i class="mdi mdi-qrcode"></i></a>
                                                            </td>
                                                            <!-- Preview Modal -->
                <div id="previewperalatanlift-<?=$user->id_peralatanlift?>" class="modal fade"  tabindex="-1" role="dialog" aria-labelledby="fullWidthModalLabel" aria-hidden="true">
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body p-4">
                                 <!-- end row -->
                                        ...
                                        <div class="row mt-4">
                                            <div class="col-sm-4">
                                                <p class="text"><strong>Nama Barang :</strong> <span class="ms-1"><?= $user->nama_peralatanlift?></span></p>
                                                <p class="text"><strong>Jenis Barang :</strong> <span class="ms-1"><?= $jenis?></span></p>

                                            <p class="text"><strong>Merek :</strong><span class="ms-1"><?= $user->merek_peralatanlift?></span></p>
                                            <p class="text"><strong>Tipe :</strong><span class="ms-1"><?= $user->tipe_peralatanlift?></span></p>
                                            <p class="text"><strong>Spesifikasi :</strong><span class="ms-1"><?= $user->spesifikasi_peralatanlift?></span></p>
                                            <p class="text"><strong>Kode Barang :</strong> <span class="ms-1"><?= $user->kode_peralatanlift?></span></p>
        ...
        <!-- Scrollable modal -->
<div class="modal fade" id="addperalatanlift" tabindex="-1" role="dialog" aria-labelledby="scrollableModalTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" height="18">Tambah Peralatan Lift</h5>
                                <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
            <div class="modal-body">
                <form class="ps-3 pe-3" action="/addperalatanlift" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-3">
                    <label for="text" class="form-label">Nama Barang</label>
                        <input class="form-control" name="nama_peralatanlift" type="text" id="nama_peralatanlift" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Jenis Barang</label>
                        <input class="form-control" name="jenis_peralatanlift" type="text" id="jenis_peralatanlift" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Merek</label>
                        <input class="form-control" name="merek_peralatanlift" type="text" id="merek_peralatanlift" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Tipe</label>
                        <input class="form-control" name="tipe_peralatanlift" type="text" id="tipe_peralatanlift" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Spesifikasi</label>
                        <textarea class="form-control" name="spesifikasi_peralatanlift" id="spesifikasi_peralatanlift" required="" rows="5" placeholder="Masukkan beberapa ringkasan tentang Spesifikasi barang.."></textarea>
                    </div>
                    <div class="mb-3">
...
<!-- Scrollable modal -->
<div class="modal fade" id="editperalatanlift" tabindex="-1" role="dialog" aria-labelledby="scrollableModalTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" height="18">Edit Peralatan Lift</h5>
                                <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
            <div class="modal-body">
                <form class="ps-3 pe-3" action="/editperalatanlift" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-3">
                    <label for="text" class="form-label">Nama Barang</label>
                        <input class="form-control" name="nama_peralatanlift" type="text" id="nameedit" required="" placeholder="">
                        <input class="form-control" name="id" type="hidden" id="idedit" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Jenis Barang</label>
                        <input class="form-control" name="jenis_peralatanlift" type="text" id="nameedit2" required="" placeholder="">
                        <input class="form-control" name="id" type="hidden" id="idedit2" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Merek</label>
                        <input class="form-control" name="merek_peralatanlift" type="text" id="nameedit3" required="" placeholder="">
                        <input class="form-control" name="id" type="hidden" id="idedit3" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Tipe</label>
                        <input class="form-control" name="tipe_peralatanlift" type="text" id="nameedit4" required="" placeholder="">
                        <input class="form-control" name="id" type="hidden" id="idedit4" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Spesifikasi</label>
                        <textarea class="form-control" name="spesifikasi_peralatanlift" id="nameedit5" required="" rows="5" placeholder="Masukkan beberapa ringkasan tentang Spesifikasi barang.."></textarea>
...
<script src="assets/js/ui/component.fileupload.js"></script>
    ...
    swal({
      ...
      if (isConfirm) {
        window.location.href = '/deleteperalatanlift/'+id;
      } else {
      }
    })
