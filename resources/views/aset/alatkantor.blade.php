                                ...
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <div class="row mb-2">
                                            <div class="row mb-2">
                                            <div class="col-sm-4">
                         @if(count($aset_alatkantor) > 1)
                            <a target="_blank" href="../qrallalatkantor/">
                        ...
                        @endif
                                            ...
                                            <div class="col-sm-8">
                                                ...
                                                <div class="text-sm-end">
                                                    <a href="../exportasetalatkantor"> <button type="button"   class="btn btn-danger mb-2 me-1"><i class="mdi mdi-file-excel me-2"></i>Excel</button></a>
                                                    <?php
                                                        if (session('data')->role == 'Corporation') {

                                                        }else {
                                                            ?>
                                                            <button type="button" data-bs-toggle="modal" data-bs-target="#addalatkantor"  class="btn btn-success mb-2 me-1"><i class="mdi mdi-plus-circle me-2"></i>Tambah</button>
                                                            <?php
                                                        }
                                                        ?>
                                            ...
                                            <div class="tab-pane show active" id="buttons-table-preview">
                                                ...
                                                <table id="datatable-buttons" class="table table-striped dt-responsive nowrap w-100">
                                                    ...
                                                    <tbody>
                                                        <?php
                                                        $nourut = 0;
                                                        foreach($aset_alatkantor as $user) {
                                                            $nourut++;
                                                            $no = $user->no_aset;
                                                            ...
                                                            <td><?= $nourut ?></td>
                                                            <td><?= $no ?></td>
                                                            <td><?= $user->nama_alatkantor ?></td>
                                                            <td><?= $jenis ?></td>
                                                            <td><?= $user->merek_alatkantor ?></td>
                                                            <td><?= $user->kode_alatkantor?></td>
                                                            ...
                                                            <a data-bs-toggle="modal" data-bs-target="#previewalatkantor-<?=$user->id_alatkantor?>" data-id='<?= $user->id_alatkantor?>' class="action-icon"> <i class="mdi mdi-eye"></i></a>
                                                            <?php
                                                        if (session('data')->role == 'Corporation') {
                                                            ...
                                                            <a onclick="loaddata(<?=$nourut?>);" data-bs-toggle="modal" data-bs-target="#editalatkantor" class="action-icon" id="editbutton-<?=$nourut?>" data-petugasdua='<?=$user->id_petugas2?>'  data-id='<?= $user->id_alatkantor ?>' data-nama='<?= $user->nama_alatkantor?>' data-jenis='<?=$jenis?>'
data-merek='<?=$user->merek_alatkantor?>'
data-tipe='<?=$user->tipe_alatkantor?>'
data-spesifikasi='<?=$user->spesifikasi_alatkantor?>'
...
data-keterangan='<?= isset($user->keterangan) ? $user->keterangan : '-'; ?>'  
                                                            ...
                                                            <a onclick="return swallnya(<?=$user->id_alatkantor?>)" href="#" class="action-icon"> <i class="mdi mdi-delete"></i></a>
                                                            <?php
                                                        }
                                                        ?>
                                                        

                                                            <a target="_blank" href="../qr22/<?=$tanggalbarang->format("Y")?>/<?=$tanggalbarang->format("m")?>/<?= $user->kode_alatkantor?>/<?= $user->id_user ?>/<?= $user->usernamanya ?>/<?=$no?>/<?= str_replace('/', ' atau ', $user->nama_alatkantor) ?>/<?= str_replace('/', ' atau ', $jenis) ?>/<?= str_replace('/', ' atau ', $merek) ?>/<?= str_replace('/', ' atau ', $tipe) ?>/<?= str_replace('/', ' atau ', $ket) ?>" class="action-icon"> <i class="mdi mdi-qrcode"></i></a>
                                                            </td>
                                                            <!-- Preview Modal -->
                <div id="previewalatkantor-<?=$user->id_alatkantor?>" class="modal fade"  tabindex="-1" role="dialog" aria-labelledby="fullWidthModalLabel" aria-hidden="true">
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body p-4">
                                 <!-- end row -->
                                        ...
                                        <div class="row mt-4">
                                            <div class="col-sm-4">
                                                <p class="text"><strong>Nama Aset :</strong> <span class="ms-1"><?= $user->nama_alatkantor?></span></p>
                                                <p class="text"><strong>Jenis Aset :</strong> <span class="ms-1"><?= $jenis?></span></p>

                                            <p class="text"><strong>Merek :</strong><span class="ms-1"><?= $user->merek_alatkantor?></span></p>
                                            <p class="text"><strong>Tipe :</strong><span class="ms-1"><?= $user->tipe_alatkantor?></span></p>
                                            <p class="text"><strong>Spesifikasi :</strong><span class="ms-1"><?= $user->spesifikasi_alatkantor?></span></p>

                                            <p class="text"><strong>Kode Aset :</strong> <span class="ms-1"><?= $user->kode_alatkantor?></span></p>
        ...
        <!-- Scrollable modal -->
<div class="modal fade" id="addalatkantor" tabindex="-1" role="dialog" aria-labelledby="scrollableModalTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" height="18">Tambah Alat Kantor</h5>
                                <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
            <div class="modal-body">
                <form class="ps-3 pe-3" action="/addalatkantor" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-3">
                    <label for="text" class="form-label">Nama Aset</label>
                        <input class="form-control" name="nama_alatkantor" type="text" id="nama_alatkantor" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Jenis Aset</label>
                        <input class="form-control" name="jenis_alatkantor" type="text" id="jenis_alatkantor" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Merek</label>
                        <input class="form-control" name="merek_alatkantor" type="text" id="merek_alatkantor" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Tipe</label>
                        <input class="form-control" name="tipe_alatkantor" type="text" id="tipe_alatkantor" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Spesifikasi</label>
                        <textarea class="form-control" name="spesifikasi_alatkantor" id="spesifikasi_alatkantor" required="" rows="5" placeholder="Masukkan beberapa ringkasan tentang Spesifikasi aset.."></textarea>
                    </div>
                    <div class="mb-3">
...
<!-- Scrollable modal -->
<div class="modal fade" id="editalatkantor" tabindex="-1" role="dialog" aria-labelledby="scrollableModalTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" height="18">Edit Alat Kantor</h5>
                                <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
            <div class="modal-body">
                <form class="ps-3 pe-3" action="/editalatkantor" method="POST"  enctype="multipart/form-data">
                    @csrf
                    <div class="mb-3">
                    <label for="text" class="form-label">Nama Aset</label>
                        <input class="form-control" name="nama_alatkantor" type="text" id="nameedit" required="" placeholder="">
                        <input class="form-control" name="id" type="hidden" id="idedit" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Jenis Aset</label>
                        <input class="form-control" name="jenis_alatkantor" type="text" id="nameedit2" required="" placeholder="">
                        <input class="form-control" name="id" type="hidden" id="idedit2" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Merek</label>
                        <input class="form-control" name="merek_alatkantor" type="text" id="nameedit3" required="" placeholder="">
                        <input class="form-control" name="id" type="hidden" id="idedit3" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Tipe</label>
                        <input class="form-control" name="tipe_alatkantor" type="text" id="nameedit4" required="" placeholder="">
                        <input class="form-control" name="id" type="hidden" id="idedit4" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Spesifikasi</label>
                        <textarea class="form-control" name="spesifikasi_alatkantor" id="nameedit5" required="" rows="5" placeholder="Masukkan beberapa ringkasan tentang Spesifikasi aset.."></textarea>
...
function swallnya(id) {
    swal({
      ...
      if (isConfirm) {
        window.location.href = '/deletealatkantor/'+id;
      } else {
      }
    })
