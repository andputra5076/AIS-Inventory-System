...
<html lang="en">
    <head>
        <meta charset="utf-8">
        <title>Aset Alat Lift | AIS</title>
    ...
    <ol class="breadcrumb bg-light-lighten p-2">
        ...
        <li class="breadcrumb-item active" aria-current="page">Alat Lift</li>
    </ol>
</nav>
                                </div>
                            </div>
                                ...
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <div class="row mb-2">
                                            <div class="row mb-2">
                                            <div class="col-sm-4">
                         @if(count($aset_alatlift) > 1)
                            <a target="_blank" href="../qrallalatlift/">
                        ...
                        @endif
                                            ...
                                            <div class="col-sm-8">
                                                ...
                                                <div class="text-sm-end">
                                                    <a href="../exportasetalatlift"> <button type="button"   class="btn btn-danger mb-2 me-1"><i class="mdi mdi-file-excel me-2"></i>Excel</button></a>
                                                    <?php
                                                        if (session('data')->role == 'Corporation') {

                                                        }else {
                                                            ?>
                                                            <button type="button" data-bs-toggle="modal" data-bs-target="#addalatlift"  class="btn btn-success mb-2 me-1"><i class="mdi mdi-plus-circle me-2"></i>Tambah</button>
                                                            <?php
                                                        }
                                                        ?>
                                                ...
                                                <table id="datatable-buttons" class="table table-striped dt-responsive nowrap w-100">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        foreach($aset_alatlift as $user) {
                                                            ...
                                                            <td><?= $nourut ?></td>
                                                            <td><?= $no ?></td>
                                                            <td><?= $user->nama_alatlift ?></td>
                                                            <td><?= $jenis ?></td>
                                                            <td><?= $user->merek_alatlift ?></td>
                                                            <td><?= $user->kode_alatlift?></td>
                                                            ...
                                                            <a data-bs-toggle="modal" data-bs-target="#previewalatlift-<?=$user->id_alatlift?>" data-id='<?= $user->id_alatlift?>' class="action-icon"> <i class="mdi mdi-eye"></i></a>
                                                            <?php
                                                        if (session('data')->role == 'Corporation') {
                                                            ...
                                                            <a onclick="loaddata(<?=$nourut?>);" data-bs-toggle="modal" data-bs-target="#editalatlift" class="action-icon" id="editbutton-<?=$nourut?>" data-petugasdua='<?=$user->id_petugas2?>' data-id='<?= $user->id_alatlift ?>' data-nama='<?= $user->nama_alatlift?>' data-jenis='<?=$jenis?>'
data-merek='<?=$user->merek_alatlift?>'
data-tipe='<?=$user->tipe_alatlift?>'
data-spesifikasi='<?=$user->spesifikasi_alatlift?>'
...
data-keterangan='<?= isset($user->keterangan) ? $user->keterangan : '-'; ?>'  
                                                            ...
                                                            <a onclick="return swallnya(<?=$user->id_alatlift?>)" href="#" class="action-icon"> <i class="mdi mdi-delete"></i></a>
                                                            <?php
                                                        }
                                                        ?>
                                                        

                                                            <a target="_blank" href="../qr22/<?=$tanggalbarang->format("Y")?>/<?=$tanggalbarang->format("m")?>/<?= $user->kode_alatlift?>/<?= $user->id_user ?>/<?= $user->usernamanya ?>/<?=$no?>/<?= str_replace('/', ' atau ', $user->nama_alatlift) ?>/<?= str_replace('/', ' atau ', $jenis) ?>/<?= str_replace('/', ' atau ', $merek) ?>/<?= str_replace('/', ' atau ', $tipe) ?>/<?= str_replace('/', ' atau ', $ket) ?>" class="action-icon"> <i class="mdi mdi-qrcode"></i></a>
                                                            </td>
                                                            <!-- Preview Modal -->
                <div id="previewalatlift-<?=$user->id_alatlift?>" class="modal fade"  tabindex="-1" role="dialog" aria-labelledby="fullWidthModalLabel" aria-hidden="true">
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body p-4">
                                 <!-- end row -->
                                        ...
                                        <div class="row mt-4">
                                            <div class="col-sm-4">
                                                <p class="text"><strong>Nama Aset :</strong> <span class="ms-1"><?= $user->nama_alatlift?></span></p>
                                                <p class="text"><strong>Jenis Aset :</strong> <span class="ms-1"><?= $jenis?></span></p>

                                            <p class="text"><strong>Merek :</strong><span class="ms-1"><?= $user->merek_alatlift?></span></p>
                                            <p class="text"><strong>Tipe :</strong><span class="ms-1"><?= $user->tipe_alatlift?></span></p>
                                            <p class="text"><strong>Spesifikasi :</strong><span class="ms-1"><?= $user->spesifikasi_alatlift?></span></p>

                                            <p class="text"><strong>Kode Aset :</strong> <span class="ms-1"><?= $user->kode_alatlift?></span></p>
        ...
        <!-- Scrollable modal -->
<div class="modal fade" id="addalatlift" tabindex="-1" role="dialog" aria-labelledby="scrollableModalTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" height="18">Tambah Alat Lift</h5>
                                <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
            <div class="modal-body">
                <form class="ps-3 pe-3" action="/addalatlift" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-3">
                    <label for="text" class="form-label">Nama Aset</label>
                        <input class="form-control" name="nama_alatlift" type="text" id="nama_alatlift" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Jenis Aset</label>
                        <input class="form-control" name="jenis_alatlift" type="text" id="jenis_alatlift" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Merek</label>
                        <input class="form-control" name="merek_alatlift" type="text" id="merek_alatlift" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Tipe</label>
                        <input class="form-control" name="tipe_alatlift" type="text" id="tipe_alatlift" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Spesifikasi</label>
                        <textarea class="form-control" name="spesifikasi_alatlift" id="spesifikasi_alatlift" required="" rows="5" placeholder="Masukkan beberapa ringkasan tentang Spesifikasi aset.."></textarea>
                    </div>
                    <div class="mb-3">
...
<!-- Scrollable modal -->
<div class="modal fade" id="editalatlift" tabindex="-1" role="dialog" aria-labelledby="scrollableModalTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" height="18">Edit Alat Lift</h5>
                                <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
            <div class="modal-body">
                <form class="ps-3 pe-3" action="/editalatlift" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-3">
                    <label for="text" class="form-label">Nama Aset</label>
                        <input class="form-control" name="nama_alatlift" type="text" id="nameedit" required="" placeholder="">
                        <input class="form-control" name="id" type="hidden" id="idedit" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Jenis Aset</label>
                        <input class="form-control" name="jenis_alatlift" type="text" id="nameedit2" required="" placeholder="">
                        <input class="form-control" name="id" type="hidden" id="idedit2" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Merek</label>
                        <input class="form-control" name="merek_alatlift" type="text" id="nameedit3" required="" placeholder="">
                        <input class="form-control" name="id" type="hidden" id="idedit3" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Tipe</label>
                        <input class="form-control" name="tipe_alatlift" type="text" id="nameedit4" required="" placeholder="">
                        <input class="form-control" name="id" type="hidden" id="idedit4" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Spesifikasi</label>
                        <textarea class="form-control" name="spesifikasi_alatlift" id="nameedit5" required="" rows="5" placeholder="Masukkan beberapa ringkasan tentang Spesifikasi aset.."></textarea>
...
function swallnya(id) {
    swal({
      ...
      if (isConfirm) {
        window.location.href = '/deletealatlift/'+id;
      } else {
      }
    })
