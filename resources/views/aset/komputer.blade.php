...
<html lang="en">
    ...
    <ol class="breadcrumb bg-light-lighten p-2">
        ...
        <li class="breadcrumb-item active" aria-current="page">Komputer</li>
    </ol>
</nav>
                                </div>
                            </div>
                                ...
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <div class="row mb-2">
                                            <div class="row mb-2">
                                            <div class="col-sm-4">
                         @if(count($aset_komputer) > 1)
                            <a target="_blank" href="../qrallkomputer/">
                        ...
                        @endif
                                            ...
                                            <div class="col-sm-8">
                                                ...
                                                <div class="text-sm-end">
                                                    <a href="../exportasetkomputer"> <button type="button"   class="btn btn-danger mb-2 me-1"><i class="mdi mdi-file-excel me-2"></i>Excel</button></a>
                                                    <?php
                                                        if (session('data')->role == 'Corporation') {

                                                        }else {
                                                            ?>
                                                            <button type="button" data-bs-toggle="modal" data-bs-target="#addkomputer"  class="btn btn-success mb-2 me-1"><i class="mdi mdi-plus-circle me-2"></i>Tambah</button>
                                                            <?php
                                                        }
                                                        ?>
                                                ...
                                                <table id="datatable-buttons" class="table table-striped dt-responsive nowrap w-100">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        foreach($aset_komputer as $user) {
                                                            ...
                                                            <td><?= $nourut ?></td>
                                                            <td><?= $no ?></td>
                                                            <td><?= $user->nama_komputer ?></td>
                                                            <td><?= $jenis ?></td>
                                                            <td><?= $user->merek_komputer ?></td>
                                                            <td><?= $user->kode_komputer?></td>
                                                            ...
                                                            <a data-bs-toggle="modal" data-bs-target="#previewkomputer-<?=$user->id_komputer?>" data-id='<?= $user->id_komputer?>' class="action-icon"> <i class="mdi mdi-eye"></i></a>
                                                            <?php
                                                        if (session('data')->role == 'Corporation') {
                                                            ...
                                                            <a onclick="loaddata(<?=$nourut?>);" data-bs-toggle="modal" data-bs-target="#editkomputer" class="action-icon" id="editbutton-<?=$nourut?>" data-petugasdua='<?=$user->id_petugas2?>' data-id='<?= $user->id_komputer ?>' data-nama='<?= $user->nama_komputer?>' data-jenis='<?=$jenis?>'
data-merek='<?=$user->merek_komputer?>'
data-tipe='<?=$user->tipe_komputer?>'
data-spesifikasi='<?=$user->spesifikasi_komputer?>'
...
data-keterangan='<?= isset($user->keterangan) ? $user->keterangan : '-'; ?>'  
                                                            ...
                                                            <a onclick="return swallnya(<?=$user->id_komputer?>)" href="#" class="action-icon"> <i class="mdi mdi-delete"></i></a>
                                                            <?php
                                                        }
                                                        ?>
                                                        

                                                            <a target="_blank" href="../qr22/<?=$tanggalbarang->format("Y")?>/<?=$tanggalbarang->format("m")?>/<?= $user->kode_komputer?>/<?= $user->id_user ?>/<?= $user->usernamanya ?>/<?=$no?>/<?= str_replace('/', ' atau ', $user->nama_komputer) ?>/<?= str_replace('/', ' atau ', $jenis) ?>/<?= str_replace('/', ' atau ', $merek) ?>/<?= str_replace('/', ' atau ', $tipe) ?>/<?= str_replace('/', ' atau ', $ket) ?>" class="action-icon"> <i class="mdi mdi-qrcode"></i></a>
                                                            </td>
                                                            <!-- Preview Modal -->
                <div id="previewkomputer-<?=$user->id_komputer?>" class="modal fade"  tabindex="-1" role="dialog" aria-labelledby="fullWidthModalLabel" aria-hidden="true">
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body p-4">
                                 <!-- end row -->
                                        ...
                                        <div class="row mt-4">
                                            <div class="col-sm-4">
                                                <p class="text"><strong>Nama Aset :</strong> <span class="ms-1"><?= $user->nama_komputer?></span></p>
                                                <p class="text"><strong>Jenis Aset :</strong> <span class="ms-1"><?= $jenis?></span></p>

                                            <p class="text"><strong>Merek :</strong><span class="ms-1"><?= $user->merek_komputer?></span></p>
                                            <p class="text"><strong>Tipe :</strong><span class="ms-1"><?= $user->tipe_komputer?></span></p>
                                            <p class="text"><strong>Spesifikasi :</strong><span class="ms-1"><?= $user->spesifikasi_komputer?></span></p>

                                            <p class="text"><strong>Kode Aset :</strong> <span class="ms-1"><?= $user->kode_komputer?></span></p>
        ...
        <!-- Scrollable modal -->
<div class="modal fade" id="addkomputer" tabindex="-1" role="dialog" aria-labelledby="scrollableModalTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" height="18">Tambah Komputer</h5>
                                <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
            <div class="modal-body">
                <form class="ps-3 pe-3" action="/addkomputer" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-3">
                    <label for="text" class="form-label">Nama Aset</label>
                        <input class="form-control" name="nama_komputer" type="text" id="nama_komputer" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Jenis Aset</label>
                        <input class="form-control" name="jenis_komputer" type="text" id="jenis_komputer" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Merek</label>
                        <input class="form-control" name="merek_komputer" type="text" id="merek_komputer" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Tipe</label>
                        <input class="form-control" name="tipe_komputer" type="text" id="tipe_komputer" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Spesifikasi</label>
                        <textarea class="form-control" name="spesifikasi_komputer" id="spesifikasi_komputer" required="" rows="5" placeholder="Masukkan beberapa ringkasan tentang Spesifikasi aset.."></textarea>
                    </div>
                    <div class="mb-3">
...
<!-- Scrollable modal -->
<div class="modal fade" id="editkomputer" tabindex="-1" role="dialog" aria-labelledby="scrollableModalTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" height="18">Edit Komputer</h5>
                                <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
            <div class="modal-body">
                <form class="ps-3 pe-3" action="/editkomputer" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-3">
                    <label for="text" class="form-label">Nama Aset</label>
                        <input class="form-control" name="nama_komputer" type="text" id="nameedit" required="" placeholder="">
                        <input class="form-control" name="id" type="hidden" id="idedit" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Jenis Aset</label>
                        <input class="form-control" name="jenis_komputer" type="text" id="nameedit2" required="" placeholder="">
                        <input class="form-control" name="id" type="hidden" id="idedit2" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Merek</label>
                        <input class="form-control" name="merek_komputer" type="text" id="nameedit3" required="" placeholder="">
                        <input class="form-control" name="id" type="hidden" id="idedit3" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Tipe</label>
                        <input class="form-control" name="tipe_komputer" type="text" id="nameedit4" required="" placeholder="">
                        <input class="form-control" name="id" type="hidden" id="idedit4" required="" placeholder="">
                    </div>
                    <div class="mb-3">
                    <label for="text" class="form-label">Spesifikasi</label>
                        <textarea class="form-control" name="spesifikasi_komputer" id="nameedit5" required="" rows="5" placeholder="Masukkan beberapa ringkasan tentang Spesifikasi aset.."></textarea>
...
<script src="assets/js/ui/component.fileupload.js"></script>
    ...
    swal({
      ...
      if (isConfirm) {
        window.location.href = '/deletekomputer/'+id;
      } else {
      }
    })
