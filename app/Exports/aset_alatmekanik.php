...
class aset_alatmekanik implements FromCollection, WithHeadings, ShouldAutoSize, WithStyles, WithEvents, WithTitle
{
    use Exportable;
    ...
    public function collection()
    {
        if (session('data')->id == '1') {
            $users = DB::table('users')
                ...
                ->select('*', 'ruangan.id as ruanganid', 'aset_alatmekanik.id_petugas2 as idusaha', 'aset_alatmekanik.id_petugas2 as idusaha', 'unit_kerja.id as idkerja', 'users.id as id_user',  'unit_kerja.name as namakerja', 'users.name as usernamanya', 'ruangan.name as ruangannama', 'petugas.name as petugasnama', 'petugas.id as idpetugas')
                ->get();
        ...
        foreach ($users as $user) {
            ...
            $spesifikasi = $user->spesifikasi_alatmekanik;
            $petugas2 = explode(',', $user->id_petugas2);
            $data->push([
                $nourut,
                $gabung,
                $no,
                $user->nama_alatmekanik,
                $merek,
                $jenis,
                $tipe,
                $spesifikasi,
